export default class LoginController {
  ...
  constructor($rootScope, $scope, $state, AuthService) {
    ...
    $scope.hasError = (fieldname) => {
      return $scope.form[fieldname].$dirty && $scope.form[fieldname].$invalid;
    };

    $scope.error = (fieldname, error) => {
      return $scope.form[fieldname].$dirty && $scope.form[fieldname].$error[error];
    };

    $scope.submittable = () => !$scope.submitting && !$scope.form.$invalid;
  }
